...
class Productdemo extends CI_Controller{
   ...
   // protected $shipping_label_file_name = '';
    ...
    public function updatecustomerinfo(){
        ...
        $state_data = $this->Csc_model->get_state_data('SC');
       ...
       //  $lowest_rate_service_code = $this->get_lowest_rate_service_code($productData,$ShipToData);
        //echo "<pre>";
        //print_r($lowest_rate_service_code);
        // exit;
    ...
    public function emailitemadminnew()
    {
        $query1 = $this->db->query("SELECT * FROM quote where token = 'hxbHnaKX72vupFjfitQBw9O1q60zc8gLCEl4kY3yPToeVUm5NJ'");
        $tokenupdate =  $query1->row_array();
        
        $query = $this->db->query("SELECT * FROM setting");
        $emailtemplete = $query->row_array();
        $option = '';
        ...
        if (isset($tokenupdate['selectedoption']) && !empty($tokenupdate['selectedoption'])) {
            ...
            $o = json_decode($tokenupdate['selectedoption']);
    ...
    {
        $query = $this->db->query("SELECT * FROM quote where token = 'hxbHnaKX72vupFjfitQBw9O1q60zc8gLCEl4kY3yPToeVUm5NJ'");
        $tokenupdate =  $query->row_array();
    ...
    function generate_shipping_label($lowest_rate_service_code,$productData,$ShipToData,$ShipFromData,$id,$postdata)
    {
        /*
        | -------------------------------------------------------------------
        | UPS Shipping API
        ...
        */
        ...
        $this->upsshipping->addField('ShipFrom_Number', $from['ShipFrom_Number']);
           $this->upsshipping->addField('test', 'test000000000');
