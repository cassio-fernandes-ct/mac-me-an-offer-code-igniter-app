...
class Product extends CI_Controller {
	...
	public function updatecustomerinfo() {
		...
		$data = $this->input->post();
		$qut_data = $this->productmodel->getquote($data['tokensavedetils']);

		if(isset( $data['estimator_form_insurance']) && $data['estimator_form_insurance'] === 'Yes') {
			$qut_price = '$' . number_format(preg_replace('/[^0-9\.]/', '', $qut_data['customerinfo']['price']) - 10, 2);
		} else {
		...
		log_message('debug', 'FX updatecustomerinfo: beginning to update ' . $qut_id);
			...
			if ($tokenupdate) {
				if (!$settingData['is_shipping_module_enabled']) {
					...
					if ($knockout['knockout'] == 0) {
						$knockoutstat = 0;
						$this->emailtem($data['tokensavedetils'], $this->shipping_label_file_name, isset( $data['estimator_form_insurance']) && $data['estimator_form_insurance'] === 'Yes');
					} else {
						$knockoutstat = 1;
					}
					//knockout email not send code.
					...
					$this->emailitemadmin($data['tokensavedetils'], $this->shipping_label_file_name, $knockoutstat, isset( $data['estimator_form_insurance']) && $data['estimator_form_insurance'] === 'Yes');
					...
					if ($productData['Weight'] <= 0 || $productData['Qty'] <= 0) {
						...
						if($_SERVER["REMOTE_ADDR"]=='50.205.49.193'){
						...
						log_message( 'debug', 'FX updatecustomerinfo: ' . json_encode( $mes ) );
	...
	public function emailitemadmin($token, $shipping_label_file_name, $knockoutstat, $insurance = false) {
		...
		if (isset($tokenupdate['customerinfo']['price']) && !empty($tokenupdate['customerinfo']['price']) && $exp_price[1] > 0) {
			$emailprice = $exp_price[1];
			$quoteprice = '<p style="color: rgb(54, 54, 54); font-family: Arial, sans-serif, Helvetica, Arial, sans-serif; font-size: 13px;"><b style="color: rgb(54, 54, 54); font-family: Arial, sans-serif, Helvetica, Arial, sans-serif;">Price: </b> $' . $emailprice . '</p>';
		}

		$htmlConten = str_replace(array('{{name}}','{{PO}}','{{phone_number}}','{{category_name}}','{{product_name}}','{{serial_number}}','{{price}}','{{options}}','{{label_image}}','Your estimate is approved and is valid for 14 days. This estimate is for the following product.','Use the attached packing instructions and prepaid label to send your product to us. We will update you once it has been received and inspected!', '{{insurance}}'),
			array('Admin', $tokenupdate['customerinfo']['id'],
				$tokenupdate['customerinfo']['form_phone_number'],
		...
		*/
		...
		// log_message( 'debug', 'FX emailitemadmin: config orig ' . print_r($config, true) );

		// Check if the string starts with the prefix
		if (strpos($config['smtp_host'], 'ssl://') === 0) {
		    // Remove the prefix
		    $config['smtp_host'] = substr($config['smtp_host'], strlen('ssl://'));
		}

		log_message( 'debug', 'FX emailitemadmin: config used ' . print_r($config, true) );

		$dns_lookup = shell_exec('dig ' . $config['smtp_host']);

		log_message( 'debug', 'FX emailitemadmin: smtp_host dns lookup: ' . $dns_lookup );
		...
		$this->email->subject('New Quote #' . $tokenupdate['customerinfo']['id'] . ' Received From ' . $tokenupdate['customerinfo']['form_first_name'] . ' ' . $tokenupdate['customerinfo']['form_last_name'] . ' ');
		...
		log_message( 'debug', 'FX emailitemadmin: sending email for PO ' . $tokenupdate['customerinfo']['id'] );

		if ($this->email->send()) {
			//echo 'Your Email has successfully been sent.';
			log_message( 'debug', 'FX emailitemadmin: email sent successfully');
		} else {
			ob_start();
			var_dump($this->email->print_debugger());
			log_message( 'debug', 'FX emailitemadmin: email send failed ' . ob_get_clean());
			show_error($this->email->print_debugger());
		}
	}

	public function emailtem($token, $shipping_label_file_name, $insurance = false) {
		...
		*/
		...
		log_message( 'debug', 'FX emailitem: sending email for PO ' . $tokenupdate['customerinfo']['id'] );

		if ($this->email->send()) {
			//echo 'Your Email has successfully been sent.';
			log_message( 'debug', 'FX emailitem: email sent successfully');
		} else {
			ob_start();
			var_dump($this->email->print_debugger());
			log_message( 'debug', 'FX emailitem: email send failed ' . ob_get_clean());
			show_error($this->email->print_debugger());
		}
	}
	...
	public function generate_shipping_label($lowest_rate_service_code, $productData, $ShipToData, $ShipFromData, $id, $postdata) {
		...
		*/
        ...
        if ($err) {
			...
			file_put_contents( APPPATH . 'third_party/shipping/shipstation.log', $data, FILE_APPEND );

            // echo "cURL Error #:" . $err;
            $mes['error'] = 'Something went wrong.';
			$mes['error_msg'] = $err;
			$mes['error_line'] = __LINE__;
			$mes['error_res'] = $response;
        ...
        if(!empty($response) && isset($response)){
			file_put_contents(APPPATH. 'third_party/hook/fedex_res_log.txt', json_encode($response));
            ...
            if(isset($response['shipmentId'])&&!empty($response['shipmentId'])&&!empty($response['labelData'])){
				...
				$PO_str = $this->productmodel->updatecustomerinfo($postdata); 
				...
				if( is_array( $response ) && isset( $response['ExceptionMessage'] ) ) {
					if( false !== strpos( $response['ExceptionMessage'], 'Unable to create FedEx shipment.' ) ) {
						$error_msg = 'Invalid address. Please make corrections and submit again.';
					}
				}
