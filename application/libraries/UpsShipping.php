...
class UpsShipping {
	...
	public function processShipAccept() {
		...
		$query = array(
			"client_id" => "kjhShoUYom2rR7iiA3alxdizMhGZxQs0pD0mCGDCG9kCxzUS",
			"redirect_uri" => "https://app.macmeanoffer.com",
			"response_type" => "code",
			"state" => "",
			"scope" => ""
		);
		...
		curl_setopt_array($curl, [
			CURLOPT_URL => "https://wwwcie.ups.com/security/v1/oauth/authorize?" . http_build_query($query),
		...
		if ($error) {
			...
			log_message('debug', 'TESTING RESPONSE: ' . print_r($response, true));
		}

		try {
			$shipmentData = $this->getProcessShipAccept();
			$shipmentData = json_encode( $shipmentData );
			log_message('debug', 'FX UPS: beginning to update ' . print_r($shipmentData, true));
			...
			log_message('debug', 'FX Ups: beginning to update ' . $settingData);
			...
			curl_close($ch);
			/* Curl End */
			log_message('debug', 'FX UPS Shipping Request ' . print_r($res, true));

			if( is_string( $res ) ) {
				$resObject = json_decode( $res );
			}
			if( isset( $resObject->Fault ) && !empty( $resObject->Fault ) ) {
				log_message('debug', 'FX UPS 403 Status Check ' . print_r($resObject, true));
				return array( $res, 403 );
			} else if( isset( $resObject->ShipmentResponse ) && !empty( $resObject->ShipmentResponse ) ) {
				log_message('debug', 'FX UPS 200 Status Check ' . print_r($resObject, true));
	...
	public function getProcessShipAccept() {
		...
		$logMessage = "Shipper Number: " . $settingData['shipper_number'];
		// echo '<pre>';
		// print_r($logMessage);
		// echo '</pre>';
  ...
  //       echo json_encode($request);
		// exit;
		log_message( 'debug', 'FX UpsShipping: ' . json_encode( $request ) );
