...
class Productmodel_beta extends CI_Model {
	...
	public function quotesteponeinsert($data) {
		...
		// file_put_contents(APPPATH.'third_party/hook/frontend_step1_quote_log.txt',print_r($data,TRUE));
		...
		if (isset($data) && !empty($data)) {
			...
			if ($row['totalnum'] == 0) {
				...
				$knockoutnumber = 0; //$this->knockout($qoteoption['option']);
	...
	*/
	...
	public function updatecustomerinfo($data) { 
		// file_put_contents(APPPATH.'third_party/hook/frontend_quote_log.txt',print_r($data,TRUE));
	...
	public function createcustomer_into_macmeoffer($data) {
		...
		$bcstoreurl = 'https://sandbox46.mybigcommerce.com/'; //$config_data['storeurl'];
		$client_id = 'eafyr7n0823o34h77umd6zgob1e4927'; //$config_data['client_id'];
		//$store_hash		= 'z7godtn57o';
		$store_hash = 'i1q814iww6'; //$config_data['storehas'];
		$auth_token = 'o309a2e1hccpvfs9vpn8cwv9ihmr7ri'; //$config_data['apitoken'];
	...
	public function thankyoupagedisplay($token, $customer_id) {
		...
		if (isset($res) && !empty($res)) {
			...
			if ($minutecount <= 30) {
				...
				$data['knockoutvalue'] = 0; //$this->knockout($data['selectedoption']);
	...
	public function logintommo($customer, $url) {
		...
		if (isset($customer) && !empty($customer)) {
			...
			$store_hash = 'i1q814iww6'; //$config_data['storehas'];
			$auth_token = 'o309a2e1hccpvfs9vpn8cwv9ihmr7ri'; //$config_data['apitoken'];
			$client_secret = 't2iu2m87u34qwcan4jj4yeo24f9jfta'; //$config_data['client_secret'];
			...
			$url = 'https://sandbox46.mybigcommerce.com/login/token/' . $mmttoken;
			$res['suc'] = $url;
		} else {
